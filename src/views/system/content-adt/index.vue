...
<script>
import { listContent_adt, getContent_adt, delContent_adt, addContent_adt, updateContent_adt } from '@/api/system/content-adt'
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listContent_adt(this.queryParams).then(response => {
        this.content_adtList = response.rows
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => ({ auditorId: item.auditorId, phStudyId: item.phStudyId }))
    ...
    handleUpdate(row) {
      ...
      var key = this.ids[0]
      if (row.auditorId) {
        key = { auditorId: row.auditorId, phStudyId: row.phStudyId }
      }
      getContent_adt(key).then(response => {
        this.form = response.data
        this.open = true
        this.title = '修改审核员审核党史内容学习的记录'
      })
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.auditorId != null) {
            updateContent_adt(this.form).then(response => {
              this.$modal.msgSuccess('修改成功')
              this.open = false
              this.getList()
            })
          } else {
            addContent_adt(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/content-adt/export', {
        ...this.queryParams
      }, `content-adt_${new Date().getTime()}.xlsx`)
    }
  }
}
</script>
